...
(function ($) {
  $.fn.extend({
      ...
      if (defaults.type == 'csv' || defaults.type == 'txt') {
		...
		//csvData=$('.csv_table').html();
		//alert($('#tableDiv').html())
		var csvData = '';
        csvData += '<html xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office"'+
        		'xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">'
		csvData += 	'<head>'
		csvData += 	'<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">'+
		...
		csvData += 	'</head>'
		csvData += 	'<body>'
		csvData += 	$('#tableDiv').html();
		csvData += 	'</body>'
		csvData += '</html>'
        try {
          var blob = new Blob([csvData], {type: "text/" + (defaults.type == 'csv' ? 'csv' : 'plain') + ";charset=utf-8"});
          saveAs(blob, defaults.fileName + '.' + 'xls', (defaults.type != 'csv' || defaults.csvUseBOM === false));
        }
        catch (e) {
        ...
        $tables.each(function(){
          ...
          $hrows.each(function() {
            ...
            ForEachVisibleCell(this, 'th,td', rowIndex, $hrows.length,
              function(cell, row, col) {
                if (cell != null) {
                  trData += '<th style="border:1px #000 solid;';
          ...
          $rows.each(function() {
            ...
            ForEachVisibleCell(this, 'td', rowIndex, $hrows.length + $rows.length,
              function(cell, row, col) {
                if (cell != null) {
                  trData += '<td style="mso-number-format:\\@;border:1px #000 solid;';
                  for (var styles in defaults.excelstyles) {
                  	console.log(defaults.excelstyles);
                    if (defaults.excelstyles.hasOwnProperty(styles)) {
                      trData += defaults.excelstyles[styles] + ': ' + $(cell).css(defaults.excelstyles[styles]) + ';';
                    }
                  }
